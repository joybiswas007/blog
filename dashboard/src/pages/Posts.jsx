...
const Posts = () => {
	...
	useEffect(() => {
		const fetchPosts = async () => {
			try {
				...
				const data = await postService.getPosts({ limit, offset, order_by, sort })
				setPosts(data.posts)
				setPagination(prev => ({
					...prev,
		...
		fetchPosts()
	}, [pagination.limit, pagination.offset, pagination.order_by, pagination.sort])
	...
	const handleSortChange = (column) => {
		setPagination(prev => ({
			...prev,
			order_by: column,
			sort: prev.order_by === column ? (prev.sort === 'DESC' ? 'ASC' : 'DESC') : 'DESC',
					...
					<>
						...
						<div className="flex items-center justify-between mt-4">
							<div className="text-sm text-gray-600 dark:text-gray-400">
								...
								{Math.min(pagination.offset + pagination.limit, pagination.totalPosts)} of{' '}

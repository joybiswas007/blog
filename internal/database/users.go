...
func (m UserModel) Insert(user *User) (int64, error) {
	...
	// to perform the insert there will be a violation of the UNIQUE "users_username_key"
...
func (m UserModel) UpdatePassword(user *User) error {
	...
	err := m.DB.QueryRow(ctx, query, user.Password.hash, user.ID).Scan(&returnedID)
	if err != nil {
		switch {
